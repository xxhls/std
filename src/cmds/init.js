...
npm run local-check`
  ...
  onFinish(async () => {
    try {
      ...
      const precommitFile = (await fs.promises.readFile(path.join(cwd, '.git/config'))).toString();
      if (!precommitFile.includes('.husky')) {
...
async function stylelint(ctx) {
  const {
    config: { type, loose, stylelintSupport },
    addDep,
    cwd,
    pkg,
  } = ctx;
...
async function eslint(ctx) {
  ...
  if (!pkg.hasInstall('eslint') || !semver.satisfies(pkg.getVersion('eslint'), '>=7.0')) {
    addDep({ name: 'eslint', dev: true, version: '^8.0.0' });
  }
  ...
  const getConfigName = () => {
    ...
    if (type === 'vue') {
      // 检测使用 vue2 还是 vue3
      return isVue3() ? `wkvue/vue3` : 'wkvue';
    }
  ...
  const config = {
    // TS 优先级高于其他
    extends: [typescript ? 'wkts' : 'wk', getConfigName()].filter(Boolean).map(i => (loose ? `${i}/loose` : i)),
    plugins: [],
    globals: {},
    rules: {},
    parserOptions: {
  ...
  if (typescript && type === 'vue') {
    ...
    Object.assign(config.parserOptions, {
      parser: '@typescript-eslint/parser',
      project: './tsconfig.json',
      // 修复指定了 project，无法检查 javascript 文件
...
async function getOptions(pkg, cwd) {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
       ...
       * @returns
       */
      when: ans => ans.gerritSupport,
      message: '请输入 Gerrit 服务器地址',
      default: 'http://gerrit.wakedata-inc.com',
    },
    {
      type: 'confirm',
      name: 'stylelintSupport',
      message: '是否启用 Stylelint 检查',
      default: false,
    },
  ]);
