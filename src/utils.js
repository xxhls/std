 ...
 *   typescriptEnable?: boolean
 *   stylelintEnable?: boolean
 *   formatPatterns?: string | string[]
 *   scriptPatterns?: string | string[]
 *   stylePatterns?: string | string[]
 *   eslintFixable?: boolean
 *   stylelintFixable?: boolean
 *   eslintArgs?: object
 *   stylelintArgs?: object
 *   prettierArgs?: object
 *   typescriptCmd?: string
 * }} Config
 */
...
const UsePnpmWorkspaceRoot = fs.existsSync('pnpm-workspace.yaml');
...
const TYPESCRIPT_SUPPORT_EXTENSIONS = ['.ts', '.tsx'];
...
function stageFiles(files) {
  if (isWindows() && files.length > 100) {
    // 分批
    const grouped = arrayGroup(files, 100);
    for (const group of grouped) {
      execCommand(`git add ${group.join(' ')}`, { printCommand: false });
    }
  } else {
 ...
 *
 * @remarks
 * 把数组按顺序拆分成数个 size 大小的数组
 *
 * @template T
 *
 * @param {T[]} array - 一个用来迭代的数组。
...
function install(deps, options = {}) {
  ...
  const toCommand = (list, dev) => {
    ...
    return UsePnpm
      ? `pnpm add ${dev ? '-D' : ''} ${UsePnpmWorkspaceRoot ? '-w' : ''} ${
          options.ignoreScripts ? '--ignore-scripts' : ''
        } ${pkgs}`
      : UseYarn
      ? `yarn add ${dev ? '-D' : ''} -W ${options.ignoreScripts ? '--ignore-scripts' : ''} ${pkgs}`
 ...
 */
function isVue3() {
  const vueInstalled = isInstalled('vue');

  if (vueInstalled) {
    const p = require('vue');
    return p.version && p.version.startsWith('3');
  }
